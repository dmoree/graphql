...
function createConnectionAndParams({
    ...
    if (field.cypherField) {
        const elementsToCollect: string[] = [];
        const matchedStatment = /(?:\s?return\s)(?<variable>\w+)/i.exec(field.cypherField.statement);
        if (!matchedStatment) throw Error("Not a valid connection statement");

        const { variable: returnedVariable } = matchedStatment.groups as { variable: string };
        const connectionNodeVariable = `${nodeVariable}_${field.cypherField.fieldName}`;
        subquery.push(
            ...[
                "CALL {",
                `WITH ${nodeVariable}`,
                `${field.cypherField.statement}`,
                "}",
                `WITH ${returnedVariable} AS ${connectionNodeVariable}`,
            ]
        );

        const node = connection.edges
            ? Object.values(connection.edges.fieldsByTypeName[field.relationshipTypeName]).find(
                  (v) => v.name === "node"
              )
            : undefined;

        const relatedNode = context.neoSchema.nodes.find((x) => x.name === field.cypherField?.typeMeta.name) as Node;
        ...
        if (whereInput) {
            ...
            const where = createConnectionWhereAndParams({
                whereInput,
                node: relatedNode,
                nodeVariable: connectionNodeVariable,
                context,
        ...
        const whereAuth = createAuthAndParams({
            ...
            entity: relatedNode,
            context,
            where: { varName: connectionNodeVariable, node: relatedNode },
        });
        if (whereAuth[0]) {
        ...
        const allowAndParams = createAuthAndParams({
            ...
            allow: {
                ...
                varName: connectionNodeVariable,
            },
        });
        if (allowAndParams[0]) {
            ...
            const sort = sortInput.map((s) =>
                [
                    ...Object.entries(s.node || []).map(
                        ([f, direction]) => `${connectionNodeVariable}.${f} ${direction}`
                    ),
                ].join(", ")
            );
            subquery.push(`WITH ${connectionNodeVariable}`);
        ...
        if (node) {
            ...
            if (nodeProjectionMeta?.authValidateStrs?.length) {
                subquery.push(
                    `CALL apoc.util.validate(NOT(${nodeProjectionMeta.authValidateStrs.join(
                        " AND "
                    )}), "${AUTH_FORBIDDEN_ERROR}", [0])`
                );
            }

            if (nodeProjectionMeta?.connectionFields?.length) {
                nodeProjectionMeta.connectionFields.forEach((connectionResolveTree) => {
                    const connectionField = relatedNode.connectionFields.find(
                        (x) => x.fieldName === connectionResolveTree.name
                    ) as ConnectionField;
                    const [nestedConnection, nestedConnectionParams] = createConnectionAndParams({
                    ...
                    nestedSubqueries.push(nestedConnection);
                    globalParams = {
                        ...globalParams,
                        ...Object.entries(nestedConnectionParams).reduce((res, [k, v]) => {
                            if (k !== `${connectionNodeVariable}_${connectionResolveTree.alias}`) {
                                res[k] = v;
                            }
                            return res;
                        }, {}),
                    };
                    if (nestedConnectionParams[`${connectionNodeVariable}_${connectionResolveTree.alias}`]) {
                        if (!nestedConnectionFieldParams) nestedConnectionFieldParams = {};
                        nestedConnectionFieldParams = {
                            ...nestedConnectionFieldParams,
                            ...{
                                [connectionResolveTree.alias]:
                                    nestedConnectionParams[`${connectionNodeVariable}_${connectionResolveTree.alias}`],
                            },
                        };
                    }
                });
        ...
        if (field.relationship.union || field.relationship.interface) {
            ...
            subquery.push("WITH collect(edge) as edges");
            subquery.push(subqueryCypher.join("\n"));
        } else {
            const relatedNodeVariable = `${nodeVariable}_${field.relationship.typeMeta.name.toLowerCase()}`;
            const relatedNode = context.neoSchema.nodes.find(
                (x) => x.name === field.relationship.typeMeta.name
            ) as Node;
